...
export default function Home() {
  ...
  return (
    <div>
        ...
        <Container maxWidth="lg">
          ...
          <Grid container spacing={3}>
            {[
              ...
              { icon: CheckCircle, title: 'Active Community', desc: 'Join thousands of earners sharing tips and celebrating achievements.' },
            ].map((item, idx) => {
              const IconComponent = item.icon;
              return (
      ...
      {/* Level Up with Daily Challenges */}
      <Box id="challenges" sx={{ py: { xs: 6, md: 8 }, backgroundColor: '#f9fafb' }}>
        <Container maxWidth="lg">
          <Typography variant="h4" sx={{ fontWeight: 800, mb: 1, textAlign: 'center', fontSize: { xs: '1.75rem', md: '2.25rem' } }}>
            Level Up with Daily Challenges
          </Typography>
          <Typography variant="body1" sx={{ textAlign: 'center', color: '#6b7280', mb: 6, fontSize: { xs: '0.95rem', md: '1rem' } }}>
            Turn every session into progress. Compete with friends, unlock streak bonuses, and cash out faster with guided challenges designed for every type of earner.
          </Typography>
          <Grid container spacing={3}>
            {[
              { icon: CheckCircle, title: 'Daily Missions', desc: 'Unlock new goals every day and keep your momentum going with personalized challenges.' },
              { icon: LocalFireDepartment, title: 'Streak Bonuses', desc: 'Maintain consistent activity to earn bonus rewards for weekly and monthly streaks.' },
              { icon: TrendingUp, title: 'Leaderboard Rewards', desc: 'Climb the community leaderboard and claim exclusive prizes for top performers.' },
              { icon: Poll, title: 'Skill-Based Quests', desc: 'Mix it up with quizzes, surveys, and creative tasks that challenge your skills.' },
            ].map((item, idx) => {
              const IconComponent = item.icon;
              return (
                <Grid item xs={12} sm={6} md={3} key={idx}>
